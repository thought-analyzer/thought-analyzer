...
trigger: ユーザーが「思考パターンを分析して」「ログを分析して」「thought analyze」「analyze my thinking」「analyze this log」と言ったとき。または「両方分析して」「dual analyze」「思考パターンとコーディング指示力を分析して」と言ったとき（この場合は後述の「デュアル分析モード」を実行する）
---
...
node C:/Users/yoshi/Documents/skills/thought-analyzer/scripts/generate-result-html.js \
  ...
  "C:/Users/yoshi/Documents/skills/thought-analyzer/result-thought-$(date +%Y%m%d-%H%M%S).html"
```

生成後：

```bash
start "C:/Users/yoshi/Documents/skills/thought-analyzer/result-thought-YYYYMMDD-HHMMSS.html"
```
...
## デュアル分析モード（思考パターン ＋ コーディング指示力）

トリガー：「両方分析して」「dual analyze」「思考パターンとコーディング指示力を分析して」

このモードでは同一ログに対して2種類の分析を順に実行し、**2つの独立したHTMLファイル**を生成する。

### 手順

1. **思考パターン分析を実行**（skill.md の Step 0〜3 に従う）
   - JSON-A を生成（`analysis_type: "thought_pattern"`）

2. **コーディング指示力分析を実行**（coding-direction-skill.md の定義に従う）
   - JSON-B を生成（`analysis_type: "coding_direction"`）

3. **両方のJSONを画面に出力**してから、送信についてまとめて確認する：
...
4. **送信処理**：選択に応じてcurlを実行する。送信はそれぞれ独立したPOSTリクエストで行い、サーバーから返却される `record_id` は別々のIDになる。両方送信する場合は以下のように報告する：
...
```bash
start "result-thought-YYYYMMDD-HHMMSS.html"
start "result-coding-YYYYMMDD-HHMMSS.html"
```

「ブラウザで2つのビジュアライザーを開きました」と報告する。
