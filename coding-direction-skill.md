...
**コーディング指示力分析 skill v1.0 が読み込まれました。**

このskillが読み取るもの・読み取らないものを最初に確認します。

**読み取るもの**
- あなた自身の発言（技術的な指示・判断・フィードバック）のみ

**読み取らないもの**
- 添付ファイル・コードブロックの内容そのもの
- AIの返答・ツールの出力・エラーメッセージの本文
- URL・固有名詞・ライブラリ名・APIキー
- 会話に含まれる機密情報や個人情報
...
含まれるもの：指示力パターン（6軸）・件数
含まれないもの：コードの内容・固有名詞・技術的な実装詳細・会話の内容

---

**データベースへの送信について（任意）**

送信すると、あなたのフィンガープリントが匿名で蓄積され、他のユーザーとの比較が将来的に可能になります。送信しなくても分析結果はこの画面で利用できます。
